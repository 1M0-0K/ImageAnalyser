((window, document, undefined) => {
  ...
  const colorSwatches = document.getElementById("swatches");
  const colorPaletteClose = document.querySelector(".color-palette-close");
  let colors = [];
  let color;
  let picking = false;
  let xPalette, yPalette;
  let movePalette = false;
  ...
  const addURL = (url) => {
    ...
    workImage.src = url;
    selectedFile.innerText = url.split("/").pop();
  ...
  const afterDisplay = () => {
    ...
    //Update the guids
    updateGuids();
    imageSize.innerText = `(${workImage.width}, ${workImage.height})`;
  ...
  const uploadFromClip = (e) => {
    ...
    if(image){
      ...
      reader.onload = function(event) {
        ...
        selectedFile.innerText = itemFile.name;
      };
  ...
  const updateGuids = () => {
    ...
    gVertical.style.width = Math.abs(canvas.width * zoom / 100) + 2 + "px";
    gHorizontal.style.height = Math.abs(canvas.height * zoom /100) + 2 + "px";

    //Update the position of the guids for the current image
    gVertical.style.left = imageX - 1 + "px";
    gHorizontal.style.top = imageY - 1 + "px";
  ...
  const zoomTool = (e) => {
    ...
    if(canWork){
      ...
      //Update positioin
      if(e.altKey && zoom >= 1 && zoom <= 1000 || zoom >= 5 && zoom <= 1000){
        imageX += (e.clientX - imageX) * zoomRatio;
        imageY += (e.clientY - imageY) * zoomRatio;
        canvas.style.left = `${imageX}px`;
        canvas.style.top = `${imageY}px`;
      }
  ...
  //Function for the actual zooming 
  const zoomToolTouch = (range, midX, midY) => {
    ...
    if(canWork){
      ...
      //
      //Update positioin
      imageX += (midX - imageX) * zoomRatio;
      imageY += (midY - imageY) * zoomRatio;
      canvas.style.left = `${imageX}px`;
      canvas.style.top = `${imageY}px`;
  ...
  const zoomToolTouchDown = (e) => {
    ...
    if(e.touches.length === 2){
      ...
      prevDifference = Math.abs(Math.sqrt(Math.pow(touchesAtOnce[1].clientX  - touchesAtOnce[0].clientX, 2) + Math.pow(touchesAtOnce[1].clientY  - touchesAtOnce[0].clientY, 2)));
  ...
  const buttonsEvents = (e) => {
    ...
    if(colorSwatches === _t && e.button === 0){
    ...
    if(colorPaletteClose.contains(_t) && e.button === 0){
      colorPalette.classList.toggle("active");
    }
  ...
  const mToolDraw = () => {
    ...
    if(touchCount === 0){
      ...
      mToolSizeLabel.style.bottom = "calc(100% - 80px)";
    }
  ...
  addEventListener("mousedown",(e) => {
    ...
    if(e.target === colorPalette){
      xPalette = e.clientX - e.target.offsetLeft;
      yPalette = e.clientY - e.target.offsetTop;
      movePalette = true;
    }
  ...
  //Add event for wen mouse move
  addEventListener("mousemove", (e) => {

    if(movePalette === true){
      colorPalette.style.left = (e.clientX - xPalette) + "px";
      colorPalette.style.top = (e.clientY - yPalette) + "px";
    }
